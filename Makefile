...
$(TARGETDIR)/gpu_transpose: ${MAIN} $(OBJECTS)
	mkdir -p $(@D)
	$(NVCC) $< $(OBJECTS) -DDATA_TYPE=$(DATA_TYPE) -DFORMAT_SPECIFIER='"$(FORMAT_SPECIFIER)"' -DTILE_DIM=$(TILE_DIM) -DBLOCK_ROWS=$(BLOCK_ROWS) -DUNROLL_FLAG=$(UNROLL_FLAG) --use_fast_math -o $@ $(INCLUDE) $(LIBS) $(if $(BANDWIDTH_PERFORMANCE),-Xptxas -$(BANDWIDTH_PERFORMANCE))
